...
const ClientRegister = () => {
    ...
    const { registerClient, verifyOTP } = useContext(AuthContext);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (!showOTP) {
                await registerClient(name.trim(), email.trim(), password);
                setShowOTP(true);
            } else {
                const data = await verifyOTP(email.trim(), otp.trim(), 'client');
                if (data.role !== 'client') {
                    setError('This OTP belongs to a non-client account.');
                    return;
                }
    ...
    return (
        <div className="max-w-md mx-auto mt-16 bg-white p-8 rounded-xl shadow-lg border border-gray-100">
            <div className="text-center mb-8">
                <h2 className="text-3xl font-extrabold text-gray-900 mb-2">Client Registration</h2>
                <p className="text-gray-500">Create your organizer account on Vireon</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                {!showOTP ? (
                    ...
                    <div>
                        <p className="text-sm text-green-700 bg-green-50 p-3 mb-4 rounded border border-green-200">
                            OTP sent successfully. Verify to activate your client account.
                        </p>
                ...
                >
                    {loading ? 'Processing...' : (showOTP ? 'Verify & Complete' : 'Register as Client')}
                </button>
            </form>
            ...
            {!showOTP && (
                <>
                    <p className="text-center mt-6 text-gray-600">
                        Already a client? <Link to="/client/login" className="text-gray-900 font-bold hover:underline">Sign in</Link>
                    </p>
                    <p className="text-center mt-2 text-gray-500 text-sm">
                        Looking for attendee signup? <Link to="/register" className="text-gray-800 font-semibold hover:underline">User registration</Link>
                    </p>
                </>
            )}
        </div>
